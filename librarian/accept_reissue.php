...
$sql = "select Category from user where RollNo='$rollno'";
...
if ($category == 'GEN' || $category == 'OBC') {
    ...
    if ($conn->query($sql1) === TRUE) {
        ...
        header("Refresh:0.01; url=reissue_requests.php", true, 303);
    } else {
        echo "<script type='text/javascript'>alert('Error')</script>";
        header("Refresh:0.01; url=reissue_requests.php", true, 303);
    }
} else {
    $sql2 = "update record set Due_Date=date_add(Due_Date,interval 180 day),Renewals_left=0 where BookId='$bookid' and RollNo='$rollno'";
